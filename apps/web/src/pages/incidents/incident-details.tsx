...
import { useIncident } from "@/hooks/api/incidents";
import {
    useEvaluations,
    useActionPlans,
    useFollowUps,
    useFormalDecisions
} from "@/hooks/api/steps";
                        ...
                        <CardContent className="space-y-4 text-sm">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <span className="font-medium">{incident.childNameFallback ?? "N/A"}</span>
                                </div>
                                <div>
                                    <span className="text-muted-foreground block">Date of Incident</span>
                                    <span className="font-medium">{incident.dateOfIncident ? format(new Date(incident.dateOfIncident), "PPP") : "N/A"}</span>
                                </div>
                            </div>
                            <div>
                                <span className="text-muted-foreground block mb-1">Description</span>
                                <p className="p-3 bg-muted rounded-md">{incident.description}</p>
                            </div>
                        </CardContent>
                    </Card>
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    {actionPlans?.length === 0 ? (
                                        <div className="text-center py-8">
                                            <p className="text-muted-foreground mb-4">No action plans yet.</p>
                                        </div>
                                    ) : (
                                        <div className="space-y-4">
                                            {actionPlans?.map(ap => (
                                                <div key={ap.id} className="border p-4 rounded-md">
                                                    <div className="text-xs text-muted-foreground mb-2">
                                                        Target Date: {ap.targetDate ? format(new Date(ap.targetDate), "PPP") : "N/A"}
                                                    </div>
                                                    <p>{ap.proposedActions}</p>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    {followUps?.length === 0 ? (
                                        ...
                                        <div className="space-y-4">
                                            {followUps?.map(fu => (
                                                <div key={fu.id} className="border p-4 rounded-md">
                                                    <div className="flex justify-between mb-2">
                                                        <span className="text-xs text-muted-foreground">
                                                            {format(new Date(fu.createdAt), "PPP")}
                                                        </span>
                                                        <Badge variant={fu.isResolved ? "default" : "destructive"}>
                                                            {fu.isResolved ? "Resolved" : "Unresolved"}
                                                        </Badge>
                                                    </div>
                                                    <p>{fu.followUpNotes}</p>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                        ...
                        <TabsContent value="decision">
                            <Card>
                                ...
                                <CardContent>
                                    {decisions?.length === 0 ? (
                                        <div className="text-center py-8">
                                            <p className="text-muted-foreground mb-4">No formal decisions yet.</p>
                                        </div>
                                    ) : (
                                        <div className="space-y-4">
                                            {decisions?.map(fd => (
                                                <div key={fd.id} className="border p-4 rounded-md">
                                                    <div className="flex justify-between mb-2">
                                                        <span className="text-xs text-muted-foreground">
                                                            {format(new Date(fd.createdAt), "PPP")}
                                                        </span>
                                                        <Badge>{fd.finalStatus}</Badge>
                                                    </div>
                                                    <div className="mb-2">
                                                        <span className="font-semibold block text-sm">Decision:</span>
                                                        <p>{fd.decisionDetails}</p>
                                                    </div>
                                                    {fd.actionsTaken && (
                                                        <div>
                                                            <span className="font-semibold block text-sm">Actions Taken:</span>
                                                            <p>{fd.actionsTaken}</p>
                                                        </div>
                                                    )}
                                                </div>
