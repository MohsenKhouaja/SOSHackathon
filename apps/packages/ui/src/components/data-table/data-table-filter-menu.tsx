...
import { DataTableRangeFilter } from "@repo/ui/components/data-table/data-table-range-filter";
...
import {
  ...
  getFilterOperators,
} from "@repo/ui/lib/data-table";
import { formatDate } from "@repo/ui/lib/format";
import { generateId } from "@repo/ui/lib/id";
import { getFiltersStateParser } from "@repo/ui/lib/parsers";
import { cn } from "@repo/ui/lib/utils";
import type {
  ExtendedColumnFilter,
  FilterOperator,
} from "@repo/ui/types/data-table";
...
// Safely render a dynamic icon component. Prevents calling DOM constructors like global Text.
type IconProps = { className?: string };
type IconLike = React.ComponentType<IconProps>;
function SafeIcon({ icon, className }: { icon?: unknown; className?: string }) {
  if (!icon) {
    return null;
  }
  // Avoid accidentally rendering the DOM Text constructor as a component
  const globalText = (globalThis as { Text?: unknown }).Text;
  if (icon === globalText || (icon as { name?: string })?.name === "Text") {
    return null;
  }
  ...
  try {
    return React.createElement(icon as IconLike, { className });
  } catch {
    return null;
  }
}
  ...
  React.useEffect(() => {
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (
        ...
        event.preventDefault();
        onFilterRemove(filters.slice(-1).pop()?.filterId ?? "");
      }
    }
                  ...
                  <CommandGroup>
                    {columns.map((column) => (
                      <CommandItem
                        ...
                        value={column.id}
                      >
                        <SafeIcon icon={column.columnDef.meta?.icon} />
