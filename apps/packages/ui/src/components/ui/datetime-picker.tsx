...
function Calendar({
  ...
  const MONTHS = React.useMemo(() => {
    let locale: Pick<Locale, "options" | "localize" | "formatLong"> = fr;
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        month: "flex flex-col items-center space-y-4 px-2",
        month_caption: "flex justify-center pt-1 relative items-center",
        caption_label: "text-sm font-medium",
        nav: "space-x-1 flex items-center ",
        button_previous: cn(
          buttonVariants({ variant: "outline" }),
          "absolute top-[1.15rem] left-5 h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100",
          disableLeftNavigation() && "pointer-events-none"
        ),
        button_next: cn(
          buttonVariants({ variant: "outline" }),
          "absolute top-[1.15rem] right-5 h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100",
      ...
      components={{
        ...
        MonthCaption: ({ calendarMonth }) => (
          <div className="inline-flex gap-2">
            ...
            >
              <SelectTrigger className="w-fit gap-1 border-border/50 p-0 px-2 focus:bg-accent focus:text-accent-foreground">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {MONTHS.map((month) => (
